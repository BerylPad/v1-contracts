...
import {IBerylPadLpLockerFeeConversion} from "./interfaces/IBerylPadLpLockerFeeConversion.sol";
...
contract BerylPadLpLockerFeeConversion is IBerylPadLpLockerFeeConversion, ReentrancyGuard, Ownable {
    ...
    constructor(
        address owner_,
        address factory_, // Address of the berylPad factory
        address feeLocker_,
        ...
        factory = factory_;
        feeLocker = IBerylPadFeeLocker(feeLocker_);
    ...
    function placeLiquidity(
        IBerylPad.LockerConfig memory lockerConfig,
        IBerylPad.PoolConfig memory poolConfig,
        ...
        FeeIn[] memory lpFeeConversionPreferences = abi.decode(
            lockerConfig.lockerData, (IBerylPadLpLockerFeeConversion.LpFeeConversionInfo)
        ).feePreference;
    ...
    function _mintLiquidity(
        ...
        for (uint256 i = 0; i < lockerConfig.tickLower.length; i++) {
            ...
            if (lockerConfig.tickLower[i] < poolConfig.tickIfToken0IsBerylPad) {
        ...
        bool token0IsBerylPad = token < poolConfig.pairedToken;
        ...
        int24 startingTick =
            token0IsBerylPad ? poolConfig.tickIfToken0IsBerylPad : -poolConfig.tickIfToken0IsBerylPad;
        ...
        for (uint256 i = 0; i < lockerConfig.tickLower.length; i++) {
            ...
            uint256 amount0 = token0IsBerylPad ? tokenAmount : 0;
            uint256 amount1 = token0IsBerylPad ? 0 : tokenAmount;

            // determine tick bounds for this position
            int24 tickLower_ =
                token0IsBerylPad ? lockerConfig.tickLower[i] : -lockerConfig.tickLower[i];
            int24 tickUpper_ =
                token0IsBerylPad ? lockerConfig.tickUpper[i] : -lockerConfig.tickUpper[i];
            int24 tickLower = token0IsBerylPad ? tickLower_ : tickUpper_;
            int24 tickUpper = token0IsBerylPad ? tickUpper_ : tickLower_;
    ...
    function _mevModuleOperating(address token) internal view returns (bool) {
        ...
        if (!IBerylPadHook(address(_tokenRewards[token].poolKey.hooks)).mevModuleEnabled(poolId)) {
            return false;
        }
        ...
        uint256 poolCreationTimestamp =
            IBerylPadHook(address(_tokenRewards[token].poolKey.hooks)).poolCreationTimestamp(poolId);
        if (
            poolCreationTimestamp
                + IBerylPadHook(address(_tokenRewards[token].poolKey.hooks)).MAX_MEV_MODULE_DELAY()
                <= block.timestamp
        ) {
            return false;
        }
        ...
        returns (uint256[] memory, uint256[] memory)
    {
        ...
        // if the rewardToken is the berylPad, we want to swap for recipients who
        // want their fee in the paired token
        //
        // conversely, if the rewardToken is the paired token, we want to swap for
        // recipients who want their fee in the berylPad token
        FeeIn toSwap = token == rewardToken ? FeeIn.Paired : FeeIn.BerylPad;
    ...
    function onERC721Received(address, address from, uint256 id, bytes calldata)
        ...
        returns (bytes4)
    {
        // Only BerylPad Factory can send NFTs here
        if (from != factory) {
            revert Unauthorized();
        }
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IERC721Receiver).interfaceId
            || interfaceId == type(IBerylPadLpLocker).interfaceId;
    }
}