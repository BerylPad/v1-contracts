...
import {IBerylPadExtension} from "../interfaces/IBerylPadExtension.sol";
import {IBerylPadPresaleAllowlist} from "./interfaces/IBerylPadPresaleAllowlist.sol";
import {IBerylPadPresaleEthToCreator} from "./interfaces/IBerylPadPresaleEthToCreator.sol";
...
contract BerylPadPresaleEthToCreator is ReentrancyGuard, IBerylPadPresaleEthToCreator, OwnerAdmins {
    IBerylPad public immutable factory;
    ...
    // berylPad fee info
    uint256 public berylPadDefaultFeeBps;
    uint256 public constant BPS = 10_000;
    address public berylPadFeeRecipient;
    ...
    constructor(address owner_, address factory_, address berylPadFeeRecipient_)
        OwnerAdmins(owner_)
    {
        factory = IBerylPad(factory_);
        _presaleId = 1;
        berylPadFeeRecipient = berylPadFeeRecipient_;
        minLockupDuration = 7 days;
        berylPadDefaultFeeBps = 500; // 5%
    }
    ...
    function setBerylPadDefaultFee(uint256 berylPadDefaultFeeBps_) external onlyOwner {
        if (berylPadDefaultFeeBps_ >= BPS) revert InvalidBerylPadFee();

        uint256 oldFee = berylPadDefaultFeeBps;
        berylPadDefaultFeeBps = berylPadDefaultFeeBps_;

        emit BerylPadDefaultFeeUpdated(oldFee, berylPadDefaultFeeBps_);
    }

    function setBerylPadFeeForPresale(uint256 presaleId, uint256 newFee)
        external
        presaleExists(presaleId)
        onlyOwner
    {
        // can only set lower
        if (newFee >= presaleState[presaleId].berylPadFee) revert InvalidBerylPadFee();

        uint256 oldFee = presaleState[presaleId].berylPadFee;
        presaleState[presaleId].berylPadFee = newFee;
        emit BerylPadFeeUpdatedForPresale(presaleId, oldFee, newFee);
    }
    ...
    function setBerylPadFeeRecipient(address recipient) external onlyOwner {
        address oldRecipient = berylPadFeeRecipient;
        berylPadFeeRecipient = recipient;
        emit BerylPadFeeRecipientUpdated(oldRecipient, recipient);
    }

    function startPresale(
        IBerylPad.DeploymentConfig memory deploymentConfig,
        uint256 minEthGoal,
        uint256 maxEthGoal,
        ...
        if (allowlist != address(0)) {
            IBerylPadPresaleAllowlist(allowlist).initialize(
        ...
        presaleState[presaleId] = Presale({
            ...
            lockupEndTime: 0,
            vestingEndTime: 0,
            berylPadFee: berylPadDefaultFeeBps
        });
        ...
        emit PresaleStarted({
            ...
            lockupDuration: lockupDuration,
            vestingDuration: vestingDuration,
            berylPadFeeBps: berylPadDefaultFeeBps
        });
    }
        ...
        nonReentrant
    {
        ...
        if (presale.allowlist != address(0)) {
            uint256 allowedAmount = IBerylPadPresaleAllowlist(presale.allowlist)
    ...
    function claimEth(uint256 presaleId, address recipient) external presaleExists(presaleId) {
        ...
        // determine fee
        uint256 fee = (presale.ethRaised * presale.berylPadFee) / BPS;
        ...
        // send eth to berylPad
        if (fee > 0) {
            (bool sent,) = payable(berylPadFeeRecipient).call{value: fee}("");
            if (!sent) revert EthTransferFailed();
        }
    ...
    function receiveTokens(
        IBerylPad.DeploymentConfig calldata deploymentConfig,
        PoolKey memory,
        address token,
        ...
        if (deploymentConfig.extensionConfigs[extensionIndex].msgValue != 0 || msg.value != 0) {
            revert IBerylPadExtension.InvalidMsgValue();
        }
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IBerylPadExtension).interfaceId;
    }
}
