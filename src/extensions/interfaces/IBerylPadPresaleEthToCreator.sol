...
interface IBerylPadPresaleEthToCreator is IBerylPadExtension {
    event PresaleStarted(
        uint256 presaleId,
        address allowlist,
        IBerylPad.DeploymentConfig deploymentConfig,
        uint256 minEthGoal,
        uint256 maxEthGoal,
    ...
    event BerylPadFeeRecipientUpdated(address oldRecipient, address recipient);
    event MinLockupDurationUpdated(uint256 oldMinLockupDuration, uint256 minLockupDuration);
    event BerylPadDefaultFeeUpdated(uint256 oldBerylPadDefaultFee, uint256 berylPadDefaultFee);
    event BerylPadFeeUpdatedForPresale(uint256 presaleId, uint256 oldBerylPadFee, uint256 berylPadFee);
    event SetAllowlist(address allowlist, bool enabled);

    struct Presale {
        PresaleStatus status; // current status of the presale
        IBerylPad.DeploymentConfig deploymentConfig; // token to be deployed upon successful presale
        ...
        uint256 berylPadFee; // berylPad's fee to take in weth on successful presale
    }
