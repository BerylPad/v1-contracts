...
abstract contract BerylPadHook is BaseHook, Ownable, IBerylPadHook {
    ...
    mapping(PoolId => bool) internal berylPadIsToken0;
    ...
    function _runMevModule(
        ...
        if (
            ...
            bool disableMevModule = IBerylPadMevModule(mevModule[poolKey.toId()]).beforeSwap(
                poolKey, swapParams, berylPadIsToken0[poolKey.toId()], mevModuleSwapData
            );
    ...
    function _lpLockerFeeClaim(PoolKey calldata poolKey) internal {
        ...
        // fee-path transfer cannot brick the swap. Mirrors the
        // _runPoolExtension try/catch: the fee claim is a non-critical side-effect,
        // so on failure we skip it and let the swap settle. Skipped fees stay
        // accrued in the position and are swept on a later swap (no value lost).
