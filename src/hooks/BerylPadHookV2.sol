...
// NOTE (Beryl fork): the upstream `import {BerylPadToken}` was a dead import — the
...
import {IBerylPadLpLocker} from "../interfaces/IBerylPadLpLocker.sol";
import {IBerylPadMevModule} from "../interfaces/IBerylPadMevModule.sol";
import {IPermit2} from "@uniswap/permit2/src/interfaces/IPermit2.sol";
import {IPoolManager} from "@uniswap/v4-core/src/interfaces/IPoolManager.sol";
import {Hooks, IHooks} from "@uniswap/v4-core/src/libraries/Hooks.sol";
import {IPositionManager} from "@uniswap/v4-periphery/src/interfaces/IPositionManager.sol";

import {IBerylPadHookV2PoolExtension} from "./interfaces/IBerylPadHookV2PoolExtension.sol";
import {IBerylPadPoolExtensionAllowlist} from "./interfaces/IBerylPadPoolExtensionAllowlist.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";

import {IBerylPadHook} from "../interfaces/IBerylPadHook.sol";
import {IBerylPadHookV2} from "./interfaces/IBerylPadHookV2.sol";
...
abstract contract BerylPadHookV2 is BaseHook, IBerylPadHookV2 {
    ...
    IBerylPadPoolExtensionAllowlist public immutable poolExtensionAllowlist;
    address public immutable weth;

    mapping(PoolId => bool) public berylPadIsToken0;
    ...
    constructor(
        ...
        factory = _factory;
        poolExtensionAllowlist = IBerylPadPoolExtensionAllowlist(_poolExtensionAllowlist);
        weth = _weth;
    }
    ...
    function _initializePoolExtensionData(
        ...
        if (_poolExtension != address(0)) {
            ...
            IBerylPadHookV2PoolExtension(_poolExtension).initializePreLockerSetup(
                poolKey, berylPadIsToken0[poolKey.toId()], poolExtensionData
            );
    ...
    function initializePool(
        ...
        // initialize the pool
        PoolKey memory poolKey =
            _initializePool(berylPad, pairedToken, tickIfToken0IsBerylPad, tickSpacing, poolData);
        ...
        emit PoolCreatedFactory({
            ...
            berylPad: berylPad,
            poolId: poolKey.toId(),
            tickIfToken0IsBerylPad: tickIfToken0IsBerylPad,
    ...
    function initializePoolOpen(
        ...
        // if able, we prefer that weth is not the berylPad as our hook fee will only
        // collect fees on the paired token
        if (berylPad == weth) {
            revert WethCannotBeBerylPad();
        }
        ...
        emit PoolCreatedOpen(
            pairedToken, berylPad, poolKey.toId(), tickIfToken0IsBerylPad, tickSpacing
        );
    ...
    function _initializePool(
        ...
        if (pairedToken == address(0) || berylPad == address(0)) {
            revert ETHPoolNotAllowed();
        }

        // determine if berylPad is token0
        bool token0IsBerylPad = berylPad < pairedToken;

        // create the pool key
        PoolKey memory _poolKey = PoolKey({
            currency0: Currency.wrap(token0IsBerylPad ? berylPad : pairedToken),
            currency1: Currency.wrap(token0IsBerylPad ? pairedToken : berylPad),
        ...
        berylPadIsToken0[_poolKey.toId()] = token0IsBerylPad;

        // initialize the pool
        int24 startingTick = token0IsBerylPad ? tickIfToken0IsBerylPad : -tickIfToken0IsBerylPad;
        ...
        onlyFactory
    {
        ...
        IBerylPadMevModule(mevModule[poolKey.toId()]).initialize(poolKey, mevModuleData);

        // give pool extension, if it exists, chance to check other configured settings
        if (poolExtension[poolKey.toId()] != address(0)) {
            IBerylPadHookV2PoolExtension(poolExtension[poolKey.toId()]).initializePostLockerSetup(
                poolKey, locker[poolKey.toId()], berylPadIsToken0[poolKey.toId()]
            );
    ...
    function _runMevModule(
        ...
        if (mevModuleOperational(poolKey.toId())) {
            ...
            bool disableMevModule = IBerylPadMevModule(mevModule[poolKey.toId()]).beforeSwap(
                poolKey, swapParams, berylPadIsToken0[poolKey.toId()], poolSwapData.mevModuleSwapData
            );
    ...
    function _runPoolExtensionHelper(
        ...
        IBerylPadHookV2PoolExtension(poolExtension[poolKey.toId()]).afterSwap(
            poolKey, swapParams, delta, berylPadIsToken0[poolKey.toId()], swapData
        );
    }
    ...
    function _lpLockerFeeClaim(PoolKey calldata poolKey) internal {
        ...
        // determine the token
        address token = berylPadIsToken0[poolKey.toId()]
    ...
    function _hookFeeClaim(PoolKey calldata poolKey) internal {
        ...
        Currency feeCurrency =
            berylPadIsToken0[poolKey.toId()] ? poolKey.currency1 : poolKey.currency0;
    ...
    function _beforeSwap(
        ...
        bool token0IsBerylPad = berylPadIsToken0[poolKey.toId()];
        bool swappingForBerylPad = swapParams.zeroForOne != token0IsBerylPad;
        bool isExactInput = swapParams.amountSpecified < 0;

        // case: specified amount paired in, unspecified amount berylPad out
        ...
        if (isExactInput && swappingForBerylPad) {
            ...
            // this also helps us match the ExactOutput swappingForBerylPad scenario
            ...
            poolManager.mint(
                address(this),
                token0IsBerylPad ? poolKey.currency1.toId() : poolKey.currency0.toId(),
                uint256(int256(fee))
            );
        }

        // case: specified amount paired out, unspecified amount berylPad in
        ...
        if (!isExactInput && !swappingForBerylPad) {
            ...
            // the ExactOutput !swappingForBerylPad scenario
            uint128 scaledProtocolFee = uint128(protocolFee) * 1e18 / (1_000_000 - protocolFee);
            int128 fee = int128(swapParams.amountSpecified * int128(scaledProtocolFee) / 1e18);
            delta = toBeforeSwapDelta(fee, 0);
    ...
    function _afterSwap(
        ...
        // case: specified amount berylPad in, unspecified amount paired out
        ...
        if (isExactInput && !swappingForBerylPad) {
            // grab non-berylPad amount out
            int128 amountOut = token0IsBerylPad ? delta.amount1() : delta.amount0();
            // take fee from it
        ...
        // case: specified amount berylPad out, unspecified amount paired in
        ...
        if (!isExactInput && swappingForBerylPad) {
            // grab non-berylPad amount in
            int128 amountIn = token0IsBerylPad ? delta.amount1() : delta.amount0();
        ...
        if (isExactInput && swappingForBerylPad || !isExactInput && !swappingForBerylPad) {
            if (berylPadIsToken0[poolKey.toId()]) {
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IBerylPadHook).interfaceId
            || interfaceId == type(IBerylPadHookV2).interfaceId;
    }
