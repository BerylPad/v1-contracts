...
import {BerylPadDeployer} from "./utils/BerylPadDeployer.sol";
...
/// @notice BerylPad Token Launcher
contract BerylPad is OwnerAdmins, ReentrancyGuard, IBerylPad {
    ...
    function setHook(address hook, bool enabled) external onlyOwnerOrAdmin {
        // check that the hook supports the IBerylPadHook interface
        if (!IBerylPadHook(hook).supportsInterface(type(IBerylPadHook).interfaceId)) {
            revert InvalidHook();
        }
    ...
    function setLocker(address locker, address hook, bool enabled) external onlyOwnerOrAdmin {
        // check that the locker supports the IBerylPadLpLocker interface
        if (!IBerylPadLpLocker(locker).supportsInterface(type(IBerylPadLpLocker).interfaceId)) {
    ...
    function setMevModule(address mevModule, bool enabled) external onlyOwnerOrAdmin {
        // check that the mev module supports the IBerylPadMevModule interface
        if (!IBerylPadMevModule(mevModule).supportsInterface(type(IBerylPadMevModule).interfaceId)) {
    ...
    function setExtension(address extension, bool enabled) external onlyOwnerOrAdmin {
        // check that the extension contract supports the IBerylPadExtension interface
        if (!IBerylPadExtension(extension).supportsInterface(type(IBerylPadExtension).interfaceId)) {
        ...
        returns (address tokenAddress)
    {
        ...
        tokenAddress = BerylPadDeployer.deployToken(tokenConfig, TOKEN_SUPPLY);
    }
        ...
        returns (address tokenAddress)
    {
        ...
        // deploy the token
        tokenAddress = BerylPadDeployer.deployToken(deploymentConfig.tokenConfig, TOKEN_SUPPLY);
        ...
        emit TokenCreated({
            ...
            poolId: poolKey.toId(),
            startingTick: deploymentConfig.poolConfig.tickIfToken0IsBerylPad,
        ...
        internal
    {
        ...
        // initialize the mev module
        IBerylPadHook(deploymentConfig.poolConfig.hook).initializeMevModule(
    ...
    function _initializePool(
        ...
        poolKey = IBerylPadHook(poolConfig.hook).initializePool(
            newToken,
            poolConfig.pairedToken,
            poolConfig.tickIfToken0IsBerylPad,
    ...
    function _initializeLiquidity(
        ...
        IBerylPadLpLocker(lockerConfig.locker).placeLiquidity(
    ...
    function _triggerExtensions(
        ...
        for (uint256 i = 0; i < deploymentConfig.extensionConfigs.length; i++) {
            ...
            IBerylPadExtension(deploymentConfig.extensionConfigs[i].extension).receiveTokens{
