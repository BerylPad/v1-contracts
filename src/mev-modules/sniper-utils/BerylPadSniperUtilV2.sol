...
import {IBerylPadSniperAuctionV0} from "../BerylPadSniperAuctionV0.sol";
...
// shared util for snipers to bid in BerylPadSniperAuction V0 and V2 auctions
contract BerylPadSniperUtilV2 is ReentrancyGuard {
    ...
    IBerylPadSniperAuctionV0 immutable berylPadSniperAuction;
    IUniversalRouter immutable universalRouter;
    IPermit2 immutable permit2;
    address immutable weth;

    constructor(
        address _berylPadSniperAuction,
        address _universalRouter,
        address _permit2,
        address _weth
    ) {
        berylPadSniperAuction = IBerylPadSniperAuctionV0(_berylPadSniperAuction);
        ...
        returns (uint256 txGasPrice)
    {
        ...
        if (desiredBidAmount % berylPadSniperAuction.paymentPerGasUnit() != 0) {
            revert InvalidBidAmount();
        }

        // calculate the gas price for the bid amount
        txGasPrice = auctionGasPeg + (desiredBidAmount / berylPadSniperAuction.paymentPerGasUnit());
    }
        ...
        nonReentrant
    {
        ...
        if (round != berylPadSniperAuction.round(poolId) || round > berylPadSniperAuction.maxRounds())
        {
            revert InvalidRound();
        }

        // check that the correct block is being bid in
        if (block.number != berylPadSniperAuction.nextAuctionBlock(poolId)) {
            revert InvalidBlock();
        }

        // check that the gas price is high enough
        int256 paymentUnits = int256(tx.gasprice) - int256(berylPadSniperAuction.gasPeg(poolId));
        if (paymentUnits < 0) {
            revert GasPriceTooLow();
        }

        // check proper msg.value was sent in
        uint256 expectedBidAmount = berylPadSniperAuction.paymentPerGasUnit() * uint256(paymentUnits);
        ...
        IERC20(weth).approve(address(berylPadSniperAuction), msg.value);
        ...
        if (
            IBerylPadHookV2(address(swapParams.poolKey.hooks)).supportsInterface(
                type(IBerylPadHookV2).interfaceId
            )
        ) {
            // v2 hook
            swapParams.hookData = abi.encode(
                IBerylPadHookV2.PoolSwapData({
        ...
        if (round + 1 != berylPadSniperAuction.round(poolId)) {
