...
import {IBerylPadFeeLocker} from "../interfaces/IBerylPadFeeLocker.sol";

import {IBerylPadHookV2} from "../hooks/interfaces/IBerylPadHookV2.sol";
...
import {IBerylPadMevDescendingFees} from "./interfaces/IBerylPadMevDescendingFees.sol";
import {IBerylPadSniperAuctionV0} from "./interfaces/IBerylPadSniperAuctionV0.sol";
...
contract BerylPadSniperAuctionV2 is
    ReentrancyGuard,
    IBerylPadSniperAuctionV0,
    IBerylPadMevDescendingFees,
    Ownable
{
    ...
    IBerylPad public immutable berylPadFactory;
    IBerylPadFeeLocker public immutable feeLocker;

    constructor(address owner_, address _berylPadFactory, address _feeLocker, address _weth)
        Ownable(owner_)
    {
        berylPadFactory = IBerylPad(_berylPadFactory);
        feeLocker = IBerylPadFeeLocker(_feeLocker);
        weth = _weth;
        ...
        view
    {
        ...
        // ensure that the associated hook is a BerylPadHookV2
        if (
            !IBerylPadHookV2(address(poolKey.hooks)).supportsInterface(
                type(IBerylPadHookV2).interfaceId
            )
        ) {
            revert OnlyBerylPadHookV2();
        }

        // ensure the starting fee is not greater than the max mev LP fee
        if (feeConfigData.startingFee > IBerylPadHookV2(address(poolKey.hooks)).MAX_MEV_LP_FEE()) {
        ...
        if (
            feeConfigData.secondsToDecay
                > IBerylPadHookV2(address(poolKey.hooks)).MAX_MEV_MODULE_DELAY()
        ) {
        ...
        onlyHook(poolKey)
    {
        ...
        IBerylPadMevDescendingFees.FeeConfig memory feeConfigData =
            abi.decode(descendingFeeConfig, (IBerylPadMevDescendingFees.FeeConfig));
    ...
    function _sendPayment(PoolKey calldata poolKey, bool berylPadIsToken0, uint256 paymentAmount)
        internal
    {
        if (paymentAmount == 0) {
            return;
        ...
        SafeERC20.safeTransfer(IERC20(weth), address(berylPadFactory), factoryPayment);

        address berylPad = berylPadIsToken0
            ? Currency.unwrap(poolKey.currency0)
            : Currency.unwrap(poolKey.currency1);

        // grab locker address from factory
        address lpLocker = berylPadFactory.tokenDeploymentInfo(berylPad).locker;

        // get reward info from the locker
        IBerylPadLpLocker.TokenRewardInfo memory tokenRewardInfo =
            IBerylPadLpLocker(lpLocker).tokenRewards(berylPad);
    ...
    function _setLpFee(PoolKey calldata poolKey, uint24 lpFee) internal {
        ...
        IBerylPadHookV2(msg.sender).mevModuleSetFee(poolKey, lpFee);
    }
    ...
    function beforeSwap(
        ...
        _sendPayment(poolKey, berylPadIsToken0, paymentAmount);
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IBerylPadMevModule).interfaceId;
    }
}
